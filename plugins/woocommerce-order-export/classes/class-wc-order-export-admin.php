...
class WC_Order_Export_Admin {
	...
	public function add_menu() {
		if ( apply_filters( 'woe_current_user_can_export', true ) ) {
			...
			} else // add after Sales Report!
			{
				...
				add_menu_page( __( 'Export Orders', 'woocommerce-order-export' ),
	...
	public function thematic_enqueue_scripts() {
		...
		if ( isset( $_REQUEST['wc_oe'] ) AND ( strpos( $_REQUEST['wc_oe'], 'add_' ) === 0 OR strpos( $_REQUEST['wc_oe'],
			...
			$localize_settings_form = array(
				'add_fields_to_export'      => __( 'Add %s fields', 'woocommerce-order-export' ),
				'repeats'                   => array(
					'rows'            => __( 'rows', 'woocommerce-order-export' ),
					'columns'         => __( 'columns', 'woocommerce-order-export' ),
					'inside_one_cell' => __( 'one row', 'woocommerce-order-export' ),
				),
				'js_tpl_popup'              => array(
					'add'                      => __( 'Add', 'woocommerce-order-export' ),
					'as'                       => __( 'as', 'woocommerce-order-export' ),
					'split_values_by'          => __( 'Split values by', 'woocommerce-order-export' ),
					'fill_order_columns_label' => __( 'Fill order columns for', 'woocommerce-order-export' ),
					'for_all_rows_label'       => __( 'all rows', 'woocommerce-order-export' ),
					'for_first_row_only_label' => __( '1st row only', 'woocommerce-order-export' ),
					'grouping_by'              => array(
						'products' => __( 'Grouping by product', 'woocommerce-order-export' ),
						'coupons'  => __( 'Grouping by coupon', 'woocommerce-order-export' ),
					),
				),
				'index'                     => array(
					'product_pop_up_title' => __( 'Set up product fields', 'woocommerce-order-export' ),
					'coupon_pop_up_title'  => __( 'Set up coupon fields', 'woocommerce-order-export' ),
					'products'             => __( 'products', 'woocommerce-order-export' ),
					'coupons'              => __( 'coupons', 'woocommerce-order-export' ),
				),
				'remove_all_fields_confirm' => __( 'Remove all fields?', 'woocommerce-order-export' ),
				'reset_profile_confirm'     => __( 'This action will reset filters, settings and fields to default state. Are you sure?', 'woocommerce-order-export' ),
			...
			$translation_array = array(
				...
				'empty_item_field'			  => __( 'select item field', 'woocommerce-order-export' ),
	...
	public function ajax_gate() {
		...
		if( !current_user_can('view_woocommerce_reports') ){
			die( __( 'You can not do it', 'woocommerce-order-export' ) );
		}

		if ( ! isset( $_REQUEST['method'] ) ) {
			die( __( 'Empty method', 'woocommerce-order-export' ) );
		}
		...
		if ( ! isset( $this->tabs[ $tab ] ) ) {
			...
			if ( ! method_exists( $ajax_handler, $method ) ) {
				die( sprintf( __( 'Unknown AJAX method %s', 'woocommerce-order-export' ), $method) );
			}
		...
		if ( ! method_exists( $this->tabs[ $tab ], $method ) ) {
			die( sprintf( __( 'Unknown tab method %s', 'woocommerce-order-export' ), $method) );
		}

		if ( $_POST && ! check_admin_referer( 'woe_nonce', 'woe_nonce' ) ) {
			die( __( 'Wrong nonce', 'woocommerce-order-export' ) );
		}
	...
	function export_orders_bulk_action( $actions ) {
		...
		if ( ! empty( $settings['format'] ) ) {
			$actions['woe_export_selected_orders'] = sprintf( __( 'Export as %s', 'woocommerce-order-export' ),
				$settings['format'] );
		}
